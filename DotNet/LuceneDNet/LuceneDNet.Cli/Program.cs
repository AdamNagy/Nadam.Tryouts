...
while (command != "exit")
{
    if (command.StartsWith("index") || command.StartsWith("idx"))
    {
        ...
        var webContentIndex = new WebContentIndex(Path.Combine(dataRoot, "contentindex", "uktights"));
        var scriptIndex = new WebContentIndex(Path.Combine(dataRoot, "contentindex", "uktights"), "script-index.txt");
        var styleIndex = new WebContentIndex(Path.Combine(dataRoot, "contentindex", "uktights"), "style-index.txt");

        var webScanner = new WebScanner(webContentIndex, scriptIndex, styleIndex, searchIndexWriter, new HttpClient());
